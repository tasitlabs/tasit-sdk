...
let wallet, provider; // Note: We're intentionally not testing the `fromEncryptedJson` or `encrypt` functions
...
describe("ethers.js", () => {
  ...
  it("should create a random wallet ", async () => {
    const randomWallet = _ethers.ethers.Wallet.createRandom();

    (0, _chai.expect)(randomWallet.address).to.have.lengthOf(42);
  });
  it("should get balance of wallet", async () => {
    let randomWallet = _ethers.ethers.Wallet.createRandom();
