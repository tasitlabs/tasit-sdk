...
export default class DecentralandUtils {
  ...
  constructor() {
    ...
    this.estate = new ethers.Contract(estateAddress, estateABI, this.provider);
  ...
  private getCreatedSellOrders = async () => {
    return this.getOrders("OrderCreated");
  };

  private getCancelledSellOrders = async () => {
    return this.getOrders("OrderCancelled");
  };

  private getExecutedSellOrders = async () => {
  ...
  private listEventLogs = async (eventABI, filter) => {
